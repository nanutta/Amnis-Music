$(function(){
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            for(let i = 0; i < response.realMe.tracklist.length; i++){
                $(`#accordion h3:eq(${i})`).html(response.realMe.tracklist[i]);
            }
            $("#projInfo h2:eq(0)").html(response.realMe.projName);
            $("#projInfo h2:eq(1)").html(response.realMe.projArtist);
            $("#projInfo p").html(response.realMe.projDesc);
            $("#projInfo img:eq(0)").attr("src", response.realMe.coverURL);
            $("#projInfo img:eq(1)").attr("src", response.realMe.profileURL);
            // AND MORE SECTION
