$(function(){
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            for(let i = 0; i < response.noah.tracklist.length; i++){
                $(`#accordion h3:eq(${i})`).html(response.noah.tracklist[i]);
            }
            $("#projInfo h2:eq(0)").html(response.noah.projName);
            $("#projInfo h2:eq(1)").html(response.noah.projArtist);
            $("#projInfo p").html(response.noah.projDesc);
            $("#projInfo img:eq(0)").attr("src", response.noah.coverURL);
            $("#projInfo img:eq(1)").attr("src", response.noah.profileURL);
            // AND MORE SECTION
